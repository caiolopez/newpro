...
var generating_copies = false  # New boolean to control copy generation
...
func _ready():
	...
	if not target or not (target is AnimatedSprite2D):
		print("Error: Target is not set or is not an AnimatedSprite2D")
		return
...
func _process(delta):
	...
	for i in range(trail_length):
		if i < len(positions):  # Check if the position exists
			...
			if opaque_copies:
				trail_sprites[i].modulate.a = 1.0
			else:
				times[i] += delta
				var alpha = 1.0 - (times[i] / fade_time)
				trail_sprites[i].modulate.a = max(0, alpha)
		else:
			trail_sprites[i].modulate.a = 0.0

func set_generating_copies(value: bool):
